...
Defines the home page hero region — the above-the-fold centrepiece every visitor sees first: a role badge, headline with an accented subheading, a short bio paragraph, a three-statistic bar, and decorative grid + radial-glow background texture. All copy is sourced from `content/home.json` at build time. The hero is a static Server Component (`app/page.tsx` → `components/home/Hero.tsx`); the chat launcher and scroll indicator are layered in by a separate capability.
