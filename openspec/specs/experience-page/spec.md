...
The `/experience` route presents Abhilash's career history as a vertical timeline so visitors and recruiters can read his roles in reverse-chronological order and download his résumé. It is a build-time SSG page driven entirely by `content/experience.json`, reusing the shared `PageHeader` and `FadeIn` page conventions.
