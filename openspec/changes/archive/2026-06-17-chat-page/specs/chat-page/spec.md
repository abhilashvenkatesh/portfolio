...
on-device WebLLM assistant whose answers are grounded in Abhilash's CV data. The page is
a client-only shell; all inference runs in the browser via WebGPU with no backend.
